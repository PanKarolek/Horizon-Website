...
function applyTheme(theme, fast = false) { 
  ...
  if (fast) { 
    void document.documentElement.offsetHeight; document.documentElement.classList.remove("no-transition");
  } }
function applyLang(lang, fast = false) { 
  ...
  updateSurveyHelpLink(lang); } applyTheme(currentTheme, true); 
